...
Sigma = diag(runif(D, 0.4, 1.6)) # diag(1L,D)
Sigma = diag(c(rep(0.1,7), 0.1,0.1, 0.1))
...
Y = MakeDiscrete(Yfull, discreteDims, groups)
...
CAVI <- function(Y, discreteDims=c(), maxiterations=1000L, tol = 0.1, seed=NULL){
  ...
  parameters$M.Z = matrix(rnorm(parameters$N*parameters$NdiscreteDims), nrow=parameters$N) #not the actual mean! only the mean in the truncated normal
  ...
  parameters$varZ = matrix(0, nrow = parameters$N, ncol= parameters$D)
  ...
  parameters$beta = parameters$alpha * (diag(var(parameters$pseudoY))) #- diag(tcrossprod(parameters$M.Lambda)))
  ...
  for (iter in 1:maxiterations){
    ...
    #parameters$pseudoY = parameters$pseudoY
    
    parameters$b = parameters$b0 + 0.5*E.Lambda2(parameters)
    
    
    #update beta (ie distribution of H)
    parameters$varZ[,discreteDims] = S.Z(parameters)
    #parameters$varZ = parameters$varZ
    for (j in 1:parameters$D){
      parameters$beta[j] =  sum(parameters$varZ[,j]) + sum(parameters$pseudoY[,j]**2) 
