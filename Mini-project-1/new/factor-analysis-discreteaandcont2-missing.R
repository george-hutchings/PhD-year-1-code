...
CAVI <- function(Y, discreteDims=c(), maxiterations=1000L, tol = 0.001, seed=NULL){
  ...
  for (i in 1:parameters$NdiscreteDims){
    ...
    print(paste('Boundaries for dim', i ))
    print(boundaries)
    parameters$pseudoY[,d] = findInterval(parameters$pseudoY[,d] - (parameters$M.Eta%*%parameters$M.Lambda[d,]), boundaries) #Todo do i subtract mean??
  }
...
if (msk==3){
  # Scaling Lambda by amount of data missing
  tmpLambda = predLambda
